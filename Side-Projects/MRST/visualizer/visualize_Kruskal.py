...
def draw_graph(data):
    ...
    for edge in edges:
        ...
        if start_pos and end_pos:
            pygame.draw.line(screen, Util.RED, start_pos, end_pos, 5)

    # plot nodes
    for node in nodes:
        scaled_pos = (int(node['x'] * Util.SCALE_FACTOR), int(Util.WINDOW_HEIGHT - node['y'] * Util.SCALE_FACTOR))
...
def draw_graph_prev(data):
    ...
    for edge in edges:
        ...
        if start_pos and end_pos:
            pygame.draw.line(screen, Util.EDGE_COLOR, start_pos, end_pos, 2)
...
while running:
    ST_files = sorted(glob.glob("graph_json/ST/*.json"), key=extract_number)
    prev_graph_data = load_graph_from_json(ST_files[-1])
    ...
    if not animation_played:
        draw_graph_prev(prev_graph_data)
        play_animation()
